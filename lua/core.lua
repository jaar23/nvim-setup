...
local function set_keymap()
	...
	-- custom keymap
	map("n", keys.update_file, "<cmd>update<cr>", {desc = "Update file"})
	map("n", keys.save_file, "<cmd>w<cr>", { desc = "Save file" })
	map("n", keys.move_down_nor, "<cmd>m .+1<cr>==", { desc = "Move down" })
	map("n", keys.move_up_nor, "<cmd>m .-2<cr>==", { desc = "Move up" })
	map("i", keys.move_down_ins, "<esc><cmd>m .+1<cr>==gi", { desc = "Move down" })
